...
SELECT t.NAME, s.END_TIME, s.SUCCESSFUL

FROM summary s

INNER JOIN tsmgui_allcli_grid t
...
SELECT t.NAME, e.END_TIME, e.SUCCESSFUL

FROM summary_extended e

INNER JOIN tsmgui_allcli_grid t

ON e.SUB_ENTITY = t.NAME

WHERE e.activity = \'BACKUP\'

AND e.END_TIME \>= (current_timestamp - 24 hours)

AND (e.ACTIVITY_TYPE = \'Full\'
...
WHERE ev.SCHEDULED_START \>= (current_timestamp - 24 hours)
