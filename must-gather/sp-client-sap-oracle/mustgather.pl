...
my $base_sap_oracle_path = env::get_sap_oracle_base_path();
unless ($base_sap_oracle_path) {
...
# Module List
# ----------------------------------
my @default_modules = qw(network system server config);
my @requested_modules = $modules ? split /,/, $modules : qw(system network config logs performance server sap-oracle);

# Combine and remove duplicates
my %seen;
my @selected_modules = grep { !$seen{$_}++ } (@default_modules, @requested_modules);

# Remove BA-dependent modules if BA client is not installed
unless ($ba_base_path) {
...
# ----------------------------------
# Interactive Oracle User (ONLY ONCE)
# ----------------------------------
my $oracle_user;

if (grep { $_ eq "sap-oracle" } @selected_modules) {
    print "\nEnter Oracle OS username for environment capture (Example: oracle or <sid>adm)\n";
...
foreach my $module (@selected_modules) {
    ...
    } elsif ($module eq "sap-oracle") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "sap-oracle.pl");
    ...
    push @args, ("--optfile", $optfile) if $optfile && ($module eq "config" || $module eq "server" || $module eq "sap-oracle");
    push @args, ("--oracle-user", $oracle_user) if $module eq "sap-oracle" && $oracle_user;
