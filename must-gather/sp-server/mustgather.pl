...
my @requested_modules = $modules ? split /,/, $modules : qw(system network server tape replication stgpool TSM-dbbackup tiering install-upgrade librarysharing oc);
...
foreach my $module (@selected_modules) {
    ...
    elsif ($module eq "TSM-dbbackup") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "TSM-dbbackup.pl");
    }
    elsif ($module eq "tiering") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "tiering.pl");
    }
    elsif ($module eq "install-upgrade"){
        $script = File::Spec->catfile($FindBin::Bin, "collector", "install_upgrade.pl");
    }
    ...
    if (($module eq "config" || $module eq "server" || $module eq "tape" || $module eq "replication" || $module eq "stgpool" || $module eq "tiering" || $module eq "librarysharing" || $module eq "oc") 
    ...
    push @args, ("-id", $adminid, "-pwd", $password)
        if ($module eq "config" || $module eq "server" || $module eq "tape" || $module eq "replication" || $module eq "stgpool" || $module eq "tiering" || $module eq "librarysharing" || $module eq "oc") && $adminid && $password;
