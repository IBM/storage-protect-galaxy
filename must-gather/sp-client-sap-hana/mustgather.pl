...
die "Error: API base path not found (bin64/bin)\n"
    unless $base_path;

my $opt_file = $optfile ? $optfile : "$base_path/dsm.opt";

die "Error: dsm.opt not found at $opt_file\n"
    unless -e $opt_file;
...
# Module List
# ----------------------------------
my @default_modules = qw(network system config);  # Always run config
my @requested_modules = $modules ? split /,/, $modules : qw(system network config logs performance server sap-hana);
...
foreach my $module (@selected_modules) {
    ...
    } elsif ($module eq "sap-hana") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "sap-hana.pl");
