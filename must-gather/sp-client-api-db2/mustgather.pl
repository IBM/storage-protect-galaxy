...
my $api_path = env::get_api_base_path();
unless ($api_path) {
...
my @requested_modules = $modules ? split /,/, $modules : qw(system network config logs performance server apidb2);
...
foreach my $module (@selected_modules) {
    ...
    } elsif ($module eq "apidb2") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "apidb2.pl");
