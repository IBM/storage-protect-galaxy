...
# Module List
# ----------------------------------
my @all_modules      = qw(system network config sapdb2 logs performance );
...
if (grep { $_ eq "network" } @selected_modules) {
    # For SAP DB2, we'll try to get server info from dsm.sys/dsm.opt
    my $dsm_config = $os =~ /MSWin32/i ? "$base_path/dsm.opt" : "$base_path/dsm.sys";
    if (-f $dsm_config) {
        $server_ip = utils::get_server_address($dsm_config, $base_path, $os);
        $port = utils::get_tcp_port($dsm_config, $base_path, $os);
    }
}
...
foreach my $module (@selected_modules) {
    ...
    elsif ($module eq "sapdb2")      { $script = "$FindBin::Bin/collector/sapdb2.pl"; }
    elsif ($module eq "logs")        { $script = "$FindBin::Bin/../sp-client-ba/collector/log.pl"; }
    elsif ($module eq "performance") { $script = "$FindBin::Bin/../sp-client-ba/collector/performance.pl"; }
    else {
