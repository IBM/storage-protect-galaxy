#  Must-Gather Scripts for IBM Storage Protect for Mail – Data Protection for Domino

# sp-client-exchange
## Overview
These scripts collect system, network, configuration, logs, server, Domino, and performance data for IBM Storage Protect for Mail – Data Protection for Microsoft Exchange Server environments.
...
- `--product, -p` : Product name (`sp-client-domino`)
...
## Example
```bash
perl mustgather.pl --product sp-client-domino --output-dir /tmp/mustgather_output --caseno TS020757841 --adminid admin --verbose
```
...
- `domino` :Collects Domino-specific diagnostic data including domdsmc query outputs (adsm, domino, preferences), Domino configuration files, and directory listings. Also gathers environment variables, user profile information (Unix/Linux/AIX), and Windows registry details.
Includes installed package information (lslpp, pkginfo, rpm) for platform verification.
