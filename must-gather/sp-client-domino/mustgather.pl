...
# ----------------------------------
my $domino_base_path = env::get_domino_base_path();
unless ($domino_base_path) {
...
my @requested_modules = $modules ? split /,/, $modules : qw(system network config logs performance server domino);
...
foreach my $module (@selected_modules) {
    ...
    } elsif ($module eq "domino") {
        $script = File::Spec->catfile($FindBin::Bin, "collector", "domino.pl");
