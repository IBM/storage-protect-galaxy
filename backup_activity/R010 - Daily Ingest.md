...
PROTECTED_GB, (CAST(FLOAT(SUM(s.bytes_written))/1024/1024/1024 AS
DECIMAL(12,2))) AS WRITTEN_GB,
(CAST(FLOAT(SUM(s.dedup_savings))/1024/1024/1024 AS DECIMAL(12,2))) AS
DEDUPSAVINGS_GB, (CAST(FLOAT(SUM(s.comp_savings))/1024/1024/1024 AS
DECIMAL(12,2))) AS COMPSAVINGS_GB,
...
AS DECIMAL(5,2))) AS COMP_PCT FROM summary s WHERE activity=\'BACKUP\'
OR activity=\'ARCHIVE\' GROUP BY DATE(S.START_TIME) ORDER BY DATE DESC;
