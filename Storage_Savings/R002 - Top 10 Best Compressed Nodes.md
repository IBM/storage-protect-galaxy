...
COALESCE((CAST(FLOAT(SUM(s.comp_savings))/FLOAT(SUM(s.bytes_protected)-SUM(s.dedup_savings))\*100
AS DECIMAL(5,2))),0) AS COMP_PCT FROM summary_extended s WHERE
activity=\'BACKUP\' OR activity=\'ARCHIVE\' GROUP BY S.ENTITY ORDER BY
COMP_PCT DESC FETCH FIRST 10 ROWS ONLY;
